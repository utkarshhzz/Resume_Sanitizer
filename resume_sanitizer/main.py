...
async def list_entities():
    """Lists all PII types this engine can detect."""
...
async def process_pdf(request: Request, file_bytes: bytes, filename: str, min_confidence: float, redaction_mode: bool, redact_types: str | None = None) -> tuple[bytes | None, SanitizeResponse]:
    """Core pipeline: parse → analyze → redact → return."""
    start_time = time.perf_counter()
    req_id = getattr(request.state, "request_id", "unknown")

    file_hash = compute_sha256(file_bytes)

    # Cache lookup (only for redaction mode)
    if redaction_mode:
    ...
    metadata = SanitizeResponse(
        request_id=req_id, filename=filename, file_hash=file_hash,
        cache_hit=False, pages_processed=len(blocks), ocr_used=ocr_used,
        pii_entities_found=len(entities), entities=entities,
...
async def sanitize_resume(
    ...
    min_confidence: float = Query(0.55, ge=0.0, le=1.0, description="Minimum confidence score"),
    redact_types: str = Query(None, description="Comma-separated list of entities to redact")
) -> Response:
    """Upload a PDF, get back a permanently sanitized version."""
    ...
    sanitized_bytes, metadata = await process_pdf(
        request=request, file_bytes=pdf_bytes,
        filename=file.filename or "upload.pdf",
        min_confidence=min_confidence, redaction_mode=True, redact_types=redact_types
    )
    ...
    return Response(content=sanitized_bytes, media_type="application/pdf", headers=headers)  # type: ignore[arg-type]
...
async def analyze_only(
    ...
    """Dry-run: detect PII without performing redaction."""
    if file.content_type not in settings.ALLOWED_CONTENT_TYPES:
        raise InvalidFileTypeError()
    ...
    _, metadata = await process_pdf(
        request=request, file_bytes=pdf_bytes,
        filename=file.filename or "upload.pdf",
        min_confidence=min_confidence, redaction_mode=False
    )
    return metadata
...
async def batch_sanitize(
    ...
    """Process up to 10 PDFs concurrently."""
    async def process_single(f: UploadFile):
        ...
        try:
            sanitized_bytes, metadata = await process_pdf(
                ...
                min_confidence=min_confidence, redaction_mode=True
            )
            encoded = base64.b64encode(sanitized_bytes).decode('utf-8') if sanitized_bytes else None  # type: ignore
            return {"metadata": metadata.model_dump(), "base64_pdf": encoded}
