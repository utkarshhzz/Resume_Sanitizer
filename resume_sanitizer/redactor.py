...
# Domains that reveal candidate identity — links to these are always severed
...
def _rects_from_word_blocks(
    ...
    """Map character offsets to word bounding boxes."""
...
def _collect_separator_rects(
    ...
    for char in SEPARATOR_SEARCH_CHARS:
        for quad in page.search_for(char, quads=True):
            r = quad.rect
            sep_rect = (r.x0, r.y0, r.x1, r.y1)
            if _rect_near_any(sep_rect, all_rects, SEPARATOR_PROXIMITY_PX):
                expanded = expand_rect(
                    sep_rect, settings.REDACTION_PADDING_PX,
                    page_rect.width, page_rect.height,
                )
                extra.append(expanded)
                all_rects.append(expanded)
...
def build_redaction_targets(
    ...
    """Translate PII entities into physical PDF rectangles for redaction."""
    ...
    for entity in entities:
        ...
        if not rects and not ocr_used:
            # Fallback: exact full-string search
        ...
        # Catch leftover separators between link redactions on the same line
...
def apply_redactions(doc: fitz.Document, targets: list[RedactionTarget]) -> bytes:
    """
    Apply permanent redactions to the PDF:
    1. White-fill over PII text (physically removes text from PDF binary)
    2. Sever all clickable hyperlinks (social media, email, personal sites)
    3. Scrub document metadata and XML metadata
    """
    ...
    for page_index in range(doc.page_count):
        ...
        # Step 1: Mark PII text areas for redaction
        ...
        # Step 2: Collect and delete hyperlinks BEFORE apply_redactions
        # (apply_redactions corrupts PyMuPDF's internal link xref table)
        try:
        ...
        for link in links:
            ...
            if uri_lower.startswith(("http://", "https://", "mailto:")):
                should_delete = True
            if should_delete:
        ...
        # Step 3: Apply all redactions in one pass
